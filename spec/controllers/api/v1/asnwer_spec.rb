...
describe 'Answers API', type: :request do
  ...
  describe 'POST /api/v1/questions/question_id/answers' do
    ...
    context 'when authorized' do
      it 'returns 200 status' do
        post request_url, params: valid_params, headers: headers.merge('Authorization' => "Bearer #{access_token.token}")
      ...
      it 'create a new answer' do
        expect { post request_url,
                      params: valid_params,
                      headers: headers.merge('Authorization' => "Bearer #{access_token.token}") }.to change(Answer, :count).by(1)
      end
    end
  end
  ...
  describe 'DELETE /api/v1/answers/answer.id' do
    ...
    context 'when authorized' do
      it 'returns 200 status' do
        delete request_url, headers: headers.merge('Authorization' => "Bearer #{access_token.token}")
